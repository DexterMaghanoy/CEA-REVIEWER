        ...
        AND quiz_type = 1
            ...
            LIMIT 1";
                                    ...
                                    if ($QuizResult) {
                                        ...
                                        $attemptSql = "SELECT COUNT(*) FROM tbl_result 
                       WHERE stud_id = :stud_id 
                         AND course_id = :course_id 
                         AND quiz_type = 2";
                                        $attemptStmt = $conn->prepare($attemptSql);
                                        $attemptStmt->bindParam(':stud_id', $stud_id);
                                        $attemptStmt->bindParam(':course_id', $course_id);
                                        $attemptStmt->execute();
                                        $quizAttempts = $attemptStmt->fetchColumn();

                                        // Get latest module_id for this course
                                        ...
                                        if ($total_questions > 0) {
                                            $scorePercent = ($result_score / $total_questions) * 100;
                                        } else {
                                            $scorePercent = 0;
                                        }
                                        ...
                                        // Display logic
                                        if ($quizAttempts > 0) {
                                            if ($scorePercent >= $passRate) {
                                                echo '<span style="color: green;"><strong>Passed</strong></span>';
                                            } else {

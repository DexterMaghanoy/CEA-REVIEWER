...
if (isset($_SESSION['program_id']) && !empty($_SESSION['program_id'])) {
    if (is_array($_SESSION['program_id'])) {
        // If program_id is an array, take the first element and force it to be an integer
        $program_id = intval($_SESSION['program_id'][0]);
    } else {
        // If program_id is not an array, force it to be an integer
        $program_id = intval($_SESSION['program_id']);
    }
} else {
    // Redirect to index.php if program_id is not set
